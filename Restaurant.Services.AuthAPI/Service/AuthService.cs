...
namespace Restaurant.Services.AuthAPI.Service
{
    public class AuthService : IAuthService
    {
        ...
        public AuthService(AppDbContext db, UserManager<ApplicationsUser> userManager, RoleManager<IdentityRole> roleManager, IJwtTokenGenerator jwtTokenGenerator)
        {
            _db = db;
        ...
        {
            var user = _db.ApplicationsUsers.FirstOrDefault(u=>u.Email.ToLower()== email.ToLower());
            if(user != null) 
            {
                if (!_roleManager.RoleExistsAsync(roleName).GetAwaiter().GetResult())
                {
                    // sreate role if it does not exist
        ...
        public async Task<string> Register(RegistrationRequestDto registrationRequestDto)
        {
            ...
            try
            {
                ...
                if (result.Succeeded)
                {
                    var userToReturn = _db.ApplicationsUsers.First(u=>u.UserName == registrationRequestDto.Email);
